...
export const storeStoreAdmissionInfo = async ({
  ...
  if (appId) {
    const qrcodeResult = await getWechatMPCode(
      appId,
      "pkg/user/pages/account/login",
      `verify:${uuid}`,
    );

    if (qrcodeResult instanceof Buffer) {
      return {
        success: true,
        data: {
          code: `data:image/png;base64,${qrcodeResult.toString("base64")}`,
        },
      };
    }

    return unknownResponse((qrcodeResult as WechatMpCodeError).errmsg);
  }
