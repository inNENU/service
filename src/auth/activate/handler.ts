...
export const activateHandler = request<ActivateResponse, ActivateOptions>(async (req, res) => {
  if (req.method === "GET") {
    const response = await getActivateInfo();

    if ("cookieStore" in response) {
      const cookies = response.cookieStore.getAllCookies().map((item) => item.toJSON());
  ...
  if (options.type === "send-sms") return res.json(await sendActivateSms(options, cookieHeader));
