...
const getStudentArchive = async (
  ...
  const study = [...studyInfo.matchAll(studyRegExp)]
    .map(([, startTime, endTime, school, title, witness, remark]) => ({
      ...
      witness: witness.replaceAll("&nbsp;", " ").trim(),
...
export const getUnderStudentArchive = async (
  ...
  const response = await fetch(`${UNDER_STUDENT_ARCHIVE_QUERY_URL}&tktime=${getIETimeStamp()}`, {
    headers: {
      Cookie: cookieHeader,
...
>(async (req, res) => {
  ...
  if (cookieHeader.includes("TEST")) return res.json(UNDER_STUDENT_ARCHIVE_VIEW_TEST_RESPONSE);
