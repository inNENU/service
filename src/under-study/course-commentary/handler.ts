...
> = async (req, res) => {
  ...
  if (req.body.type === "list") {
    if (cookieHeader.includes("TEST")) return res.json(UNDER_COURSE_COMMENTARY_LIST_TEST_RESPONSE);

    return res.json(await listUnderCourseCommentary(cookieHeader, req.body.time));
  }
