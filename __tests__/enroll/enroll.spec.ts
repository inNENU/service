...
describe("招生 /enroll", () => {
  ...
  it("研究生招生计划 POST /enroll/grad-plan", async () => {
    const res = await client.post("/enroll/grad-plan");

    if (!expectSuccess(res, "/enroll/grad-plan", ["closed"])) return;
    ...
    for (const plan of res.body.data) {
      ...
      expectArrayItems(plan.majors, ["name", "code", "type", "content"], "grad-plan majors");
    }
  });
