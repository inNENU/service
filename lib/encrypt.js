...
const Base = (C_lib.Base = {
  ...
  /**
   * Initializes a newly created object. Override this method to add some logic when your objects
   * are created.
   *
   * @example
   *   var MyType = CryptoJS.lib.Base.extend({
 ...
 *
 * @property {number} _minBufferSize The number of blocks that should be kept unprocessed in the
 *   buffer. Default: 0
 */
const BufferedBlockAlgorithm = (C_lib.BufferedBlockAlgorithm = Base.extend({
   ...
   *
   * @param {WordArray | string} data The data to append. Strings are converted to a WordArray using
   *   UTF-8.
   */
 ...
 *
 * @property {number} blockSize The number of 32-bit words this hasher operates on. Default: 16 (512
 *   bits)
 */
const Hasher = (C_lib.Hasher = BufferedBlockAlgorithm.extend({
  ...
  /**
   * Finalizes the hash computation. Note that the finalize operation is effectively a destructive,
   * read-once operation.
   *
   * @example
...
const Cipher = (C_lib.Cipher = BufferedBlockAlgorithm.extend({
   ...
   * @returns {Cipher} A cipher instance.
   * @static
   */
   ...
   *
   * @example
   *   var cipher = CryptoJS.algo.AES.create(CryptoJS.algo.AES._ENC_XFORM_MODE, keyWordArray, {
   *     iv: ivWordArray,
   *   });
   *
   ...
   * @returns {WordArray} The data after processing.
   */
   ...
   * a destructive, read-once operation.
   *
   * @example
   ...
   * @returns {WordArray} The data after final processing.
   */
 ...
 *
 * @property {number} blockSize The number of 32-bit words this cipher operates on. Default: 1 (32
 *   bits)
 */
 ...
 *
 * @property {number} blockSize The number of 32-bit words this cipher operates on. Default: 4 (128
 *   bits)
 */
 ...
 * @property {Format} formatter The default formatting strategy to convert this cipher params object
 *   to a string.
 */
...
var SerializableCipher = (C_lib.SerializableCipher = Base.extend({
   ...
   * Configuration options.
   *
   * @property {Formatter} format The formatting strategy to convert cipher param objects to and
   *   from a string. Default: OpenSSL
   */
  cfg: Base.extend({
  ...
  /**
   * Converts serialized ciphertext to CipherParams, else assumed CipherParams already and returns
...
/**
 * A serializable cipher wrapper that derives the key from a password, and returns ciphertext as a
 * serializable cipher params object.
 */
